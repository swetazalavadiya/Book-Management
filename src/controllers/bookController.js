...
const bookDelete = async (req, res) => {
    ...
    try {
        ...
        let validId = await bookModels.findById(bookId)
        if (!validId) return res.status(404).send({ status: true, message: "required validId" })

        let books=await bookModels.findOneAndUpdate(
            { _id: bookId , isDeleted: false },
            { $set: { isDeleted: true, deletedAt: new Date() } },
            { new: true });
            
        if (!books) return res.status(404).send({ message: "this is wrong id" })
        return res.status(200).send({ status: true, message: "Book Deleted Successfully", data:books})
