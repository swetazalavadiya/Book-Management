...
const registerUser = async function(req, res) {
  try {
    ...
    if (address) {
      ...
      if (city) {
        if (!validName(city)) return res.status(400).send({ status: false, message: "Enter a valid city" });
      }
...
const loginUser = async function (req, res) {
  try {
    ...
    let getUser = await userModel.findOne({ email: email, password:password }).select({ password: 1 })    
